...
/**
 * @module OneDriveLargeFileUploadTaskExt
 */
...
import {
    ...
    getValidRangeSize, GraphClientError,
    LargeFileUploadSession,
    LargeFileUploadTask,
    LargeFileUploadTaskOptions, UploadEventHandlers
...
/**
 * @class
 * Class representing OneDriveLargeFileUploadTaskExt
 */
export class OneDriveLargeFileUploadTaskExt<T> extends LargeFileUploadTask<T> {
    /**
     * @private
     * @static
     ...
     * @param {path} [path = OneDriveLargeFileUploadTaskExt.DEFAULT_UPLOAD_PATH] - The path for the upload
     * @param {drive} [drive = OneDriveLargeFileUploadTaskExt.DEFAULT_DRIVE]  - The drive identifier/name for the upload
     * @returns The constructed create session url
     */
    private static constructCreateSessionUrl(fileName: string,
                                             path: string = OneDriveLargeFileUploadTaskExt.DEFAULT_UPLOAD_PATH,
                                             drive: string = OneDriveLargeFileUploadTaskExt.DEFAULT_DRIVE): string
     ...
     * @static
     * @async
     * Creates a OneDriveLargeFileUploadTaskExt
     * @param {Client} client - The GraphClient instance
     * @param {Blob | Uint8Array | File} file - File represented as Blob, Uint8Array or File
     * @param {OneDriveLargeFileUploadOptions} options - The options for upload task
     * @returns The promise that will be resolves to OneDriveLargeFileUploadTaskExt instance
     */
    public static async create(client: Client, file: Blob | Uint8Array | File, options: OneDriveLargeFileUploadOptions): Promise<OneDriveLargeFileUploadTaskExt<Blob | ArrayBuffer | Uint8Array>> {
        ...
        const fileInfo = OneDriveLargeFileUploadTaskExt.getFileInfo(file, fileName);
     ...
     * @returns The promise that will be resolves to OneDriveLargeFileUploadTaskExt instance
     */
    public static async createTaskWithFileObject<T>(client: Client, fileObject: FileObject<T>, options: OneDriveLargeFileUploadOptions): Promise<OneDriveLargeFileUploadTaskExt<T>> {
        ...
        const requestUrl = OneDriveLargeFileUploadTaskExt.constructCreateSessionUrl(options.fileName, options.drive, options.path);
        const uploadSessionPayload: OneDriveFileUploadSessionPayLoad = {
            fileName: options.fileName,
            fileDescription: options.fileDescription,
            conflictBehavior: options.conflictBehavior,
        };
        const session = await OneDriveLargeFileUploadTaskExt.createUploadSession(client, requestUrl, uploadSessionPayload);
        const rangeSize = getValidRangeSize(options.rangeSize);
        return new OneDriveLargeFileUploadTaskExt(client, fileObject, session, {
            rangeSize,
     ...
     * @public
     * @constructor
     * Constructs a OneDriveLargeFileUploadTaskExt
     * @param {Client} client - The GraphClient instance
     * @param {FileObject} file - The FileObject holding details of a file that needs to be uploaded
     * @param {LargeFileUploadSession} uploadSession - The upload session to which the upload has to be done
     * @param {LargeFileUploadTaskOptions} options - The upload task options
     * @returns An instance of OneDriveLargeFileUploadTaskExt
     */
